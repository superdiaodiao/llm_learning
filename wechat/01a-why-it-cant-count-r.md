...
> 公众号版 **第 1 课**。对应仓库 [01 · Token 与 Embedding](../01-tokens-and-embeddings.md) 的前半段：文字怎么被切碎。面向零基础读者，约 1300 字。
...
现在你去试，不少模型已经能答对了。但如果你让它把思考过程写出来，多半会看到这样一件事：**它先把这个单词一个字母一个字母地拆开，摆成 s-t-r-a-w-b-e-r-r-y，然后才开始数。**

（这道题太有名，有些模型已经直接记住了答案，那就看不到拆的过程。换一个生僻点的长单词再问一次，过程往往就露出来了。）
