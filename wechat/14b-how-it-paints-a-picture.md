...
- 找一个画图工具，同一句描述，把"提示词相关性"或"引导强度"调到 2 和 15 各画一次。低的自由散漫，高的贴描述但颜色发腻。
- 让它画一张带文字的招牌，看字对不对。多半不对，那是压缩器糊掉的细节。
