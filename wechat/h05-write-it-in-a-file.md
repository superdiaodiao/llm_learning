...
- **项目文件**，放在项目文件夹里。跟着项目走：谁打开这个项目，AI 都会读到它。写项目的规矩。
- **个人文件**，放在你电脑的用户主目录里。跟着你走：你打开任何项目，AI 都会读到它。写你自己的习惯。
...
| Cursor | `.cursor/rules/` 里的规则文件 | 设置里的 User Rules |

（`~` 就是你的用户主目录，Mac 上是 `/Users/你的名字`，Windows 上是 `C:\Users\你的名字`。）
...
**1. 在项目根目录建一个文件。** 新建 `CLAUDE.md`（Codex 是 `AGENTS.md`）；或者在对话里输入 `/init`，它会看一遍项目自动生成一份初稿，你再删改。Claude Code 和 Codex 都有这个命令。

**2. 只写四类东西。** 怎么跑（确切的命令）、目录在哪（五到十行）、跟别处不一样的约定、禁区。判断标准一句话：**它自己发现不了、又每次都需要的**，才写。

**3. 个人偏好放个人文件。** "用中文回复""别客套"这类是你的习惯，不是项目的规矩。放个人文件里，只对你生效。项目文件是全组人共用的，别把自己的口味写进去，不然同事打开项目也会被要求用中文回复。
