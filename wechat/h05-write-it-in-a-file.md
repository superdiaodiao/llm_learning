...
工具留了一个口子：**一个固定的文件，每一轮开始时自动读进窗口。**写进去的话，你就再也不用说了。三个常用工具里它叫不同的名字，是同一样东西：

- **Claude Code**：项目里 `CLAUDE.md`，个人的在 `~/.claude/CLAUDE.md`
- **Codex**：项目里 `AGENTS.md`，个人的在 `~/.codex/AGENTS.md`
- **Cursor**：项目里 `.cursor/rules/` 下的规则文件（老版本是 `.cursorrules`），个人的在设置里的 User Rules
...
**3. 个人偏好放个人文件。** "用中文回复""别客套"这类是你的习惯，不是项目的规矩。放上面列的个人文件里，只对你生效。项目文件是全组人共用的，别把自己的口味写进去。
