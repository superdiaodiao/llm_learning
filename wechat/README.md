...
| `04d-finetune-or-retrieve.md` | 第 9 课 | 04 后半段（微调、LoRA）+ 10 开头（微调能做什么），预告 RAG |
...
- **点名读者认识的东西。**钩子里、正文关键处、"自己试一下"里，要出现读者每天用的产品和新闻里的热词（ChatGPT、豆包、DeepSeek、Claude Code、Manus、MCP、Agent……）。原理不变，例子落到具体产品上。只写公开的、稳定的事实，不猜产品内部实现。
...
课程的读者在新闻读者里。每篇新闻结尾挂一条相关的课，话题对照表在 `CROSSLINK.md`，
可粘贴块：`python3 wechat/build-crosslink.py 7`。课程不当新闻的次条；用四周后的合集累计和点赞率衡量，不看首日阅读。
