...
模型一次只吐一个 token，没法"回头"。那它怎么解决需要多步的问题？答案朴素得出人意料：**把思考过程也当成 token 吐出来**——先生成一段草稿（拆解、试算、检查），再给最终答案。这叫思维链（chain of thought）。它有效，是因为每一步都只是一次简单的下一词预测，难题被拆成了一串容易的预测。代价是思考也要花 token，所以推理模式更慢、更贵。

关键问题是：怎么让模型**想得好**？靠手写示范（SFT）只能教它模仿，教不出它自己没见过的解法。o1 和 DeepSeek-R1 走的是另一条路：**带可验证奖励的强化学习（RLVR）**。给它大量有标准答案的题（数学、代码），让它自己生成思考过程和答案，答对给奖励、答错不给，然后用梯度下降往"答对的那类思考"上挪。**不告诉它怎么想，只告诉它想对了没有。** 常用算法是 GRPO：对同一道题采样一组回答，用组内相对好坏当信号，省掉了 RLHF 里那个单独的价值模型。

结果是涌现出来的。R1 的论文里记录了模型在训练中自发学会了回头检查、换思路、写下"等等，让我重新看看"——这些行为没有任何人示范过。

这一步仍然是同一个动作：算 loss、梯度下降、挪权重。只是信号不再来自"像不像人写的"，而是来自"答案对不对"。

## 模型的一生

把五步串起来：

> 随机权重 →（预训练：万亿 token 上梯度下降）→ 会预测语言的基座模型 →（微调 / LoRA：小数据上继续练）→ 专精某任务 →（对齐 / DPO：按人类偏好调）→（推理 RL：按答案对错调）→ 你用的聊天助手。

五步都是同一个动作：**算 loss，梯度下降，挪权重。** 变的只是"用什么数据、调哪些权重、loss 怎么定义"。
...
- 推理模型把思考过程当 token 吐出来；用可验证奖励的强化学习（RLVR / GRPO）教它想得好——只告诉它答对没有，不告诉它怎么想。
- 预训练、微调、对齐、推理 RL 本质是同一个动作的不同配置。

## 思考题

LoRA 冻结了原权重 W，只训练小矩阵 B 和 A。为什么它几乎不损失效果？训练时它省下的显存具体是哪一部分，又有哪一部分省不掉？（提示："低秩改动量"；以及"梯度和优化器状态"与"激活值"的区别。）

推理 RL 只用"答案对不对"当信号，为什么这比手写思考过程示范（SFT）更能教出好的推理？（提示：SFT 的上限是什么。）
