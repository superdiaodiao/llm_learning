...
Why does this work? Because the adjustment fine-tuning needs is in fact "low-rank": you don't need the expressive power of the whole big matrix, the product of two thin ones is enough. The result is tens of times less GPU memory, so a consumer-grade card can fine-tune a large model. The real-code textbook is HuggingFace's `peft` library.
...
- Fine-tuning continues training on top of pre-training; LoRA freezes the original weights and trains two small low-rank matrices, saving tens of times the memory.
...
LoRA freezes the original weight W and trains only the small matrices B and A. Why does it lose almost no quality while saving tens of times the GPU memory? (Hint: the phrase "low-rank delta".)
